...
function bumpBucket(bucket, key, date, tokens) {
  let bucket_ = bucket[key]
  if (!bucket_) bucket_ = bucket[key] = { tokens: 0, req: 0, days: {} }
  bucket_.tokens += tokens
  bucket_.req += 1
  bucket_.days[date] = (bucket_.days[date] || 0) + tokens
}
...
function aggregate(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (event.type === 'request/header') {
      ...
      bumpBucket(providers, provider, date, tokens)
      bumpBucket(models, model + '@' + provider, date, tokens)
...
const EMPTY = { days: {}, providers: {}, models: {}, longestTurnMs: 0, span: { ms: 0, date: null }, sessions: {}, lines: 0 }
...
function mergeInto(target, agg) {
  ...
  for (const kind of ['providers', 'models']) {
    for (const key of Object.keys(agg[kind] || {})) {
      ...
      const dst = target[kind][key] || (target[kind][key] = { tokens: 0, req: 0, days: {} })
...
function summarize(agg, generatedAt) {
  ...
  const top = (bucket) =>
    Object.keys(bucket)
      .map((id) => ({ id, tokens: bucket[id].tokens, req: bucket[id].req, days: Object.keys(bucket[id].days || {}).length }))
...
// v2: day/bucket tokens = input + output only (prompt-cache reads are excluded from
// the headline metric). Bump this whenever the formula changes so stale caches rescan.
...
export function createUsageTracker({ dshHome, cacheFile, onError }) {
  ...
  async function scan() {
    ...
    const live = {}
    const merged = { days: {}, providers: {}, models: {}, longestTurnMs: 0, span: { ms: 0, date: null }, sessions: {}, lines: 0 }
    let rescanned = 0
