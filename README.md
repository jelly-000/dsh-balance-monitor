...
| **Token 活动看板** | 5 张统计卡（累计、峰值、最长会话时长、当前连续天数、最长连续天数）+ 52 周每日 token 热力图，支持每日 / 每周 / 累计三档切换，并附厂商与模型明细表 |
...
点击 `▦ Token 活动` 打开看板。数据全部来自本地 `$DSH_HOME/sessions/*/*/session.jsonl[.zstd]`，不调用任何厂商接口，因此「本地实测」型厂商同样有数据。

- **累计 Token 数**：所有会话的 in + out 之和（缓存读取仅计入 tooltip 与脚注，推理 token 不计入；采用 `亿` / `万` 中文计数）
- **峰值 Token 数**：单日最高值，标注日期
- **最长会话时长**：单次连续对话的最长时长（相邻事件间隔超过 30 分钟即断开，避免将跨天挂机计为会话）
- **当前 / 最长连续天数**：打卡式连续活跃天数
- **热力图**：52 周（约一年）GitHub 贡献墙样式，周一对齐，含月份轴与星期轴，颜色按 `sqrt` 分 5 档；支持每日 / 每周 / 累计切换，单元格 tooltip 展示当天 tokens / 请求 / 轮次 / 时长

解析说明：dsh 的 `.zstd` 文件为多帧拼接流，`zstdDecompressSync` 仅解出第一帧；插件按 magic `28 B5 2F FD` 切帧逐帧解压再拼接（实测 32621 帧，0 失败）。按 `size + mtimeMs` 生成指纹缓存（`$DSH_HOME/storages/balance-monitor-usage.json`），冷扫约 2 秒、命中约 300 毫秒；重启后先以缓存摘要秒开，再于后台重扫。
